                        ...
                        if "attachments" in item:
                            for attachment in item['attachments']:
                                if attachment['type'] == 'audio':
                                    responseaudio = vk.method('audio.getById',{'audios': str(attachment['audio']['owner_id'])+'_'+str(attachment['audio']['id'])})
                                    time.sleep(2)
                                    linkitem = DownloadLinkItem()
                                    if len(responseaudio) > 0:
                                        if 'url' in responseaudio[0] and 'artist' in responseaudio[0] and 'title' in responseaudio[0] and 'duration' in responseaudio[0] and 'id' in attachment['audio'] and 'owner_id' in attachment['audio']:
                                            ...
                                            linkitem['aid'] = str(attachment['audio']['id'])
